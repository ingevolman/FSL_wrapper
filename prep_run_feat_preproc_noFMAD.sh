...
# This script prepares data for preprocessing through feat, it prepares a feat.fsf file for the preprocessing
# and it runs that subject specific feat.fsf file.
# This script is written to be called by batch_script

######### GENERAL ########################################################################################

# retrieve subject & task info from first command line argument (./$scriptDir/prep_run_feat_preproc ${run})
subj_id=$1
task=$2
#$1 refers to the first command line argument, so if you typed ./batchFSL 30933 into the command line then 30933 would be the value of subj_id.
#subj_id=103_F03

# create variable for subject directory
...
# Ref for publication: Zhang, Y. and Brady, M. and Smith, S. Segmentation of brain MR images through a hidden Markov random \
# field model and the expectation-maximization algorithm. IEEE Trans Med Imag, 20(1):45-57, 2001.)
...
for i in /vols/Scratch/ivolman/data_${task}_work/'template_noFMAP.fsf'; do
